...
class Command implements CommandType.Type {
  ...
  #action: ((argument?: any, options?: any) => any) | undefined;
  #name: string;
  #description: string;
  ...
  constructor(program: ProgramType.Type, name: string, description: string, config?: CommandType.Config) {
  ...
  command(name: string, description: string, config?: CommandType.Config): CommandType.Type {
    const result = this.#program.command(this.#name + " " + name, description, config);
    this.#nested.push(this.#program.getAllCommands().length - 1);
    return result;
  }

  argument(synopsis: string, description: string, callback?: (cls: ArgumentType.Type) => any): CommandType.Type {
  ...
  option(synopsis: string, description: string, callback?: (cls: OptionType.Type) => any): CommandType.Type {
  ...
  action(callback: (options?: any, argument?: any) => any): CommandType.Type {
  ...
  async execute(data?: any): Promise<any> {
    if (this.#action) {
      if (data.argument && Object.keys(data.argument) && data.options) return await this.#action(data.argument, data.options);
      else if (data.argument && Object.keys(data.argument)) return await this.#action(data.argument);
  ...
  showHelp(stdout: boolean = true): string {
    ...
    const help = new Help(this.#name, this.#description, infoProgram.config?.version || "1.0.0", {
